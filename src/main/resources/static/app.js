var app = angular.module('transformPdf', []);
...
app.service('uploadService', [ '$http', '$rootScope', function($http, $rootScope) {
	this.search = function(name, date) {
		$http.get("http://localhost:8080/transform/uploadFile", {
			params : {
				person : name,
				date : date
			}
...
app.service('fileUpload', ['$http','uploadService', function($http, ArchiveService) {
...
app.controller('UploadCtrl', [ '$scope', 'fileUpload',
		function($scope, fileUpload) {
			$scope.uploadFile = function() {
				...
				var uploadUrl = "/transform/uploadFile";
...
app.controller('ArchiveCtrl', function($scope, $http) {
	$scope.search = function(name, date) {
		$http.get("http://localhost:8080/transform/documents", {
			params : {
				person : name,
				date : date
			}
...
app.run(function($rootScope, $http) {
	$http.get("http://localhost:8080/t/documents").success(
			function(response) {
				$rootScope.metadataList = response;
			});
});
